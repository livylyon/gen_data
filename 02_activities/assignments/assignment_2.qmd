...
```{r}
fam <- fread("02_activities/data/gwa.A2.fam")
nrow(fam)
```
...
```{r}
bim <- fread("02_activities/data/gwa.A2.bim")
nrow(bim)
```
...
ggplot(fam, aes(x = V6)) +
  ...
  labs(
    title = "Distribution of Phenotype",
    x = "Phenotype Value",
    y = "Frequency"
  )
```

The phenotype distribution appears to be approximately Gaussian, with a bell-shaped curve centered around the mean (normal). There are no obvious signs of skewness or heavy tails, suggesting that the data is reasonably normally distributed.
...
plink2 \
  ...
  --make-bed \
  --out 02_activities/data/gwa.qc.A2_unrelated
```
...
```{bash}
plink2 \
  --bfile 02_activities/data/gwa.qc.A2_unrelated \
  --extract 02_activities/data/gwa.qc.A2_pruned.prune.in \
  --make-bed \
  --out 02_activities/data/gwa.qc.A2_unrelated_pruned

plink2 \
  --bfile 02_activities/data/gwa.qc.A2_unrelated_pruned \
  --pca 20 \
  --out 02_activities/data/gwa.A2_unrel_PCA
```
...
```{bash}
plink2 --bfile ./02_activities/data/gwa.qc.A2_unrelated --ci 0.95 --linear --covar ./02_activities/data/gwa.A2_unrel_PCA.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./02_activities/data/gwa.A2_assoc
```
...
assoc <- fread("02_activities/data/gwa.A2_assoc.PHENO1.glm.linear")
